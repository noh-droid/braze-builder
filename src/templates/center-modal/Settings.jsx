...
import {
  ...
  SliderInput, SelectInput, AlignButtons, CheckboxInput,
  PopupImageUpload,
} from "../../components/ui/index.jsx";

export default function CenterModalSettings({ config, onUpdate, activeTab }) {
  return (
    <>
      {activeTab === "contents" && (
        <>
          ...
          <SliderInput value={config.bgRadius} onChange={v => onUpdate("bgRadius", v)} min={0} max={30} />
          <FieldLabel>Background Image URL</FieldLabel>
          <TextInput value={config.bgImage} onChange={v => onUpdate("bgImage", v)} placeholder="https://..." />
          ...
          >
            <div style={{
              ...
              <span style={{ fontWeight: "600", color: "#475569" }}>형식:</span> JPG, PNG, WEBP&nbsp;&nbsp;|&nbsp;&nbsp;
              <span style={{ fontWeight: "600", color: "#475569" }}>권장 사이즈:</span> 590×590px (2x 레티나)<br/>
              <span style={{ fontWeight: "600", color: "#475569" }}>비율:</span> 1:1 정사각형&nbsp;&nbsp;|&nbsp;&nbsp;
              <span style={{ fontWeight: "600", color: "#475569" }}>용량:</span> 200KB 이하
            </div>
          </PopupImageUpload>
          <FieldLabel>Image Radius</FieldLabel>
          ...
          <SelectInput value={config.titleWeight} onChange={v => onUpdate("titleWeight", v)} options={WEIGHT_OPTIONS} />
          <FieldLabel>Text</FieldLabel>
          <TextInput value={config.titleText} onChange={v => onUpdate("titleText", v)} />
          <FieldLabel>Size</FieldLabel>
          <SliderInput value={config.titleSize} onChange={v => onUpdate("titleSize", v)} min={10} max={36} />
          <FieldLabel>Color</FieldLabel>
          <ColorInput value={config.titleColor} onChange={v => onUpdate("titleColor", v)} />
          <FieldLabel>Align</FieldLabel>
          <AlignButtons value={config.titleAlign} onChange={v => onUpdate("titleAlign", v)} />

          <SectionTitle>Subtitle</SectionTitle>
          <FieldLabel>Font</FieldLabel>
          <SelectInput value={config.subFont} onChange={v => onUpdate("subFont", v)} options={FONT_OPTIONS} />
          <FieldLabel>Weight</FieldLabel>
          <SelectInput value={config.subWeight} onChange={v => onUpdate("subWeight", v)} options={WEIGHT_OPTIONS} />
          <FieldLabel>Text</FieldLabel>
          <TextInput value={config.subText} onChange={v => onUpdate("subText", v)} multiline />
          <FieldLabel>Size</FieldLabel>
          <SliderInput value={config.subSize} onChange={v => onUpdate("subSize", v)} min={10} max={24} />
          <FieldLabel>Color</FieldLabel>
          <ColorInput value={config.subColor} onChange={v => onUpdate("subColor", v)} />
          <FieldLabel>Align</FieldLabel>
          <AlignButtons value={config.subAlign} onChange={v => onUpdate("subAlign", v)} />
        </>
      )}
