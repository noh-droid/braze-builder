...
export default function FigmaSlideVerticalSettings({ config, onUpdate, activeTab }) {
  return (
    <>
      {activeTab === "contents" && (
        <>
          ...
          <SectionTitle>슬라이드 (세로형)</SectionTitle>
          <FieldLabel>슬라이드 너비 (px)</FieldLabel>
          <SliderInput value={config.slideImageWidth ?? 270} onChange={v => onUpdate("slideImageWidth", v)} min={200} max={300} />
          <FieldLabel>슬라이드 높이 (px)</FieldLabel>
          <SliderInput value={config.slideImageHeight ?? 348} onChange={v => onUpdate("slideImageHeight", v)} min={200} max={400} />
          <div style={{ fontSize: "11px", color: "#64748b", marginBottom: "8px" }}>
            최대 3장까지 등록 가능합니다. 등록한 장수만큼 슬라이드가 생성됩니다. 스와이프로만 넘깁니다.
          </div>
      ...
      {activeTab === "braze" && (
        <>
          ...
          <div style={{
            ...
            Figma <strong>Slide_Modal_Vertical</strong> (14:481) — 가로 스크롤 슬라이드 + 제목/부제 + CTA.
          </div>
        </>
      )}
    </>
