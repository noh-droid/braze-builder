...
import { isFullModalWidth, scaleDesignPx } from "../../constants/modalViewport.js";

const s = scaleDesignPx;

export default function FigmaModalPreview({ config }) {
  const rowFull = isFullModalWidth(config.btnRowWidth ?? 339);
  const btnH = s(config.btnHeight ?? 46);
  const btnGap = s(config.btnGap ?? 9);
  const rowPad = s(config.btnRowPadding ?? 20);
  const bottomOff = s(config.btnOverlayBottom ?? 0);
  const radius = s(config.imageRadius ?? 14);
  const br = s(config.btnRadius ?? 4);
  const btnFs = Math.max(10, s(config.btnSize ?? 15));
          ...
          >
            ...
            >
              <img
                ...
                onError={(e) => {
                  e.target.src = "https://placehold.co/600x600/e2e8f0/94a3b8?text=Image";
                }}
              />
              {(config.buttonCount ?? 2) !== 0 && (
                <div
                  style={{
                    ...
                    width: rowFull ? "100%" : `${s(config.btnRowWidth)}px`,
                    maxWidth: "100%",
                    padding: `${rowPad}px`,
