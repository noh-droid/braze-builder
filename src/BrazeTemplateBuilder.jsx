...
export default function BrazeTemplateBuilder() {
  ...
  const handleTemplateChange = useCallback((id) => {
    ...
    setConfig(next.normalizeConfig ? next.normalizeConfig(next.defaultConfig) : next.defaultConfig);
    setActiveTab("contents");
    setShowCode(false);
    setSelectorOpen(false);
  }, [setSelectedTemplateId, setConfig, setActiveTab, setShowCode, setSelectorOpen]);

  const configForTemplate = useMemo(() => {
    if (typeof template.normalizeConfig === "function") {
      return template.normalizeConfig(config);
    }
    return config;
  }, [template, config]);

  const generatedCode = template.generateCode(configForTemplate);
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
        ...
        <div style={{
          ...
          {showCode ? (
            ...
            <Preview key={selectedTemplateId} config={configForTemplate} />
          )}
        </div>
        ...
        <div style={{
          ...
          <div style={{ flex: 1, overflow: "auto", padding: "0 16px 24px 16px" }}>
            <Settings config={configForTemplate} onUpdate={update} activeTab={activeTab} />
          </div>
        </div>
      </div>
    </div>
